    ...
    F: Fn(u64, u64) -> f32 + Sync,
{
    ...
    let mut partition_scores: Vec<(usize, f32)> = ivf.centroids.iter().enumerate()
        .map(|(i, center)| {
    ...
    let target_partitions: Vec<usize> = partition_scores.iter().take(ivf.config.nprobe).map(|(i, _)| *i).collect();
    ...
    F: Fn(u64, u64) -> f32 + Sync,
{
    ...
    let all_candidates: Vec<Vec<(u64, f32)>> = partitions.par_iter()
        .map(|&part_id| {
            ...
            match partition.search(0, k * 2, distance_fn) { // 0 is Layer 0? NO!
    ...
    // Sort final results
    let mut sorted_results = heap.into_sorted_vec(); // Ascending order (smallest first)?
    ...
    let results = sorted_results.into_iter().map(|item| HybridSearchResult {
        id: item.id,
        score: item.score,
    }).collect();
