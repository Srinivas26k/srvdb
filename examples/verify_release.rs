use srvdb::{SvDB, IndexMode, Vector, VectorEngine};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if Path::new(path_flat).exists() { std::fs::remove_dir_all(path_flat)?; }
    ...
    println!("   -> Ingested {} vectors in {:.2}ms", n_vecs, start.elapsed().as_millis());
    
    let query = &vecs[0];
    let results = db.search(query, 5)?;
    println!("   -> Search results for vector 0: {:?}", results.iter().map(|r| r.id).collect::<Vec<_>>());
    ...
    if Path::new(path_sq).exists() { std::fs::remove_dir_all(path_sq)?; }
    ...
    // So we must add them.
    println!("   -> Trained SQ8 in {:.2}ms", start_train.elapsed().as_millis());
    
    let start_add = Instant::now();
    db_sq.add_batch(&vecs, &ids)?;
    db_sq.persist()?; // Flush
    println!("   -> Ingested (Quantized) in {:.2}ms", start_add.elapsed().as_millis());
    
    let results_sq = db_sq.search(query, 5)?;
    println!("   -> Search results (SQ8): {:?}", results_sq.iter().map(|r| r.id).collect::<Vec<_>>());
